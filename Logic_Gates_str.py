...
all_points = {'A':1,'B':2,'C':3,'P':4,'Q':5,'R':6,'S':7,'T':8,'U':9,'V':10,'W':11,'a':1,'b':2,'c':3,'p':4,'q':5,'r':6,'s':7,'t':8,'u':9,'v':10,'w':11, 'NOT':0,'AND':1,'OR':2,'NAND':3,'NOR':4,'XOR':5,'not':0,'and':1,'or':2,'nand':3,'nor':4,'xor':5,'Not':0,'And':1,'Or':2,'Nand':3,'Nor':4,'Xor':5}
	...
	if counter < gate_count:
		...
		if len(small_statement) == 2:
			while (small_statement[0] not in NOT and len(small_statement) < 3) or small_statement[1] not in all_points:
				...
				small_statement = small_statement.split()
		if len(small_statement) != 2:
			while len(small_statement) == 1 or len(small_statement) > 3 or small_statement[1] not in gates_dict or small_statement[0] not in all_points or small_statement[2] not in all_points:
